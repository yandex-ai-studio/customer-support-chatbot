...
class AirlineStateManager:
    """Manages per-profile airline customer state."""

    def __init__(self) -> None:
        self._states: Dict[str, CustomerProfile] = {}

    def _create_default_state(self, profile_id: str) -> CustomerProfile:
        segments = [
            FlightSegment(
                flight_number="OA476",
                date="2025-10-02",
    ...
    def get_profile(self, profile_id: str) -> CustomerProfile:
        if profile_id not in self._states:
            self._states[profile_id] = self._create_default_state(profile_id)
